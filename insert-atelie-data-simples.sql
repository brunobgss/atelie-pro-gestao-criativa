-- =====================================================
-- SCRIPT SIMPLES PARA INSERIR DADOS POR EMPRESA
-- =====================================================
-- Execute este script no SQL Editor do Supabase

-- 1. Primeiro, vamos pegar o ID da empresa existente
-- (assumindo que existe apenas uma empresa)
SELECT id as empresa_id FROM empresas LIMIT 1;

-- 2. Inserir orçamentos para a empresa existente
INSERT INTO atelie_quotes (code, customer_name, customer_phone, customer_email, date, observations, total_value, status, empresa_id) 
SELECT 
    'ORC-001',
    'Roberto Alves',
    '(11) 99999-1111',
    'roberto@empresa.com',
    '2025-01-09',
    'Camisetas bordadas para empresa',
    850.00,
    'pending',
    (SELECT id FROM empresas LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM atelie_quotes WHERE code = 'ORC-001');
...
SELECT 
    ...
    'Escola Municipal',
    '(11) 99999-2222',
    'contato@escola.edu.br',
    '2025-01-08',
    'Uniformes escolares completos',
    3500.00,
    'approved',
    (SELECT id FROM empresas LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM atelie_quotes WHERE code = 'ORC-002');
...
SELECT 
    ...
    'pending',
    (SELECT id FROM empresas LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM atelie_quotes WHERE code = 'ORC-003');

-- 3. Inserir itens dos orçamentos
